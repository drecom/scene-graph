...
class FbxLoader implements ThreeLoader {
    public static threeRef: any;
    ...
    constructor(manager?: any) {
        this.manager = (manager !== undefined) ? manager : FbxLoader.threeRef.DefaultLoadingManager;
    }
    load(url: any, onLoad: any, onProgress?: any, onError?: any) {
        var path = (this.path === undefined) ? FbxLoader.threeRef.LoaderUtils.extractUrlBase(url) : this.path;
        var loader = new FbxLoader.threeRef.FileLoader(this.manager);
    ...
    parse(FBXBuffer: any, path: any) {
        ...
        var textureLoader = new FbxLoader.threeRef.TextureLoader(this.manager).setPath(this.resourcePath || path).setCrossOrigin(this.crossOrigin);
...
class FBXTreeParser {
    ...
    parse(fbxTree: any) {
        ...
        FbxLoader.threeRef.Loader.Handlers.add(/\.tga$/i, tgaLoader);
    ...
    parseTexture(textureNode: any, images: any) {
        ...
        texture.wrapS = valueU === 0 ? FbxLoader.threeRef.RepeatWrapping : FbxLoader.threeRef.ClampToEdgeWrapping;
        texture.wrapT = valueV === 0 ? FbxLoader.threeRef.RepeatWrapping : FbxLoader.threeRef.ClampToEdgeWrapping;
    ...
    loadTexture(textureNode: any, images: any) {
        ...
        if (extension === 'tga') {
            const loader = FbxLoader.threeRef.Loader.Handlers.get('.tga');
            if (loader === null) {
                console.warn('FbxLoader: TgaLoader not found, creating empty placeholder texture for', fileName);
                texture = new FbxLoader.threeRef.Texture();
            } else {
    ...
    parseMaterial(materialNode: any, textureMap: any) {
        ...
        switch (type.toLowerCase()) {
            case 'phong':
                material = new FbxLoader.threeRef.MeshPhongMaterial();
                break;
            case 'lambert':
                material = new FbxLoader.threeRef.MeshLambertMaterial();
                break;
            default:
    ...
    parseParameters(materialNode: any, textureMap: any, ID: any) {
        ...
        if (materialNode.Diffuse) {
            parameters.color = new FbxLoader.threeRef.Color().fromArray(materialNode.Diffuse.value);
        } else if (materialNode.DiffuseColor && materialNode.DiffuseColor.type === 'Color') {
            // The blender exporter exports diffuse here instead of in materialNode.Diffuse
            parameters.color = new FbxLoader.threeRef.Color().fromArray(materialNode.DiffuseColor.value);
        }
        if (materialNode.DisplacementFactor) {
            parameters.displacementScale = materialNode.DisplacementFactor.value;
        }
        if (materialNode.Emissive) {
            parameters.emissive = new FbxLoader.threeRef.Color().fromArray(materialNode.Emissive.value);
        } else if (materialNode.EmissiveColor && materialNode.EmissiveColor.type === 'Color') {
            // The blender exporter exports emissive color here instead of in materialNode.Emissive
            parameters.emissive = new FbxLoader.threeRef.Color().fromArray(materialNode.EmissiveColor.value);
        }
        ...
        if (materialNode.Specular) {
            parameters.specular = new FbxLoader.threeRef.Color().fromArray(materialNode.Specular.value);
        } else if (materialNode.SpecularColor && materialNode.SpecularColor.type === 'Color') {
            // The blender exporter exports specular color here instead of in materialNode.Specular
            parameters.specular = new FbxLoader.threeRef.Color().fromArray(materialNode.SpecularColor.value);
        }
        connections.get(ID).children.forEach((child: any) => {
            ...
            switch (type) {
                ...
                case 'ReflectionColor':
                    ...
                    parameters.envMap.mapping = FbxLoader.threeRef.EquirectangularReflectionMapping;
                    break;
                case 'SpecularColor':
    ...
    parseSkeleton(relationships: any, deformerNodes: any) {
        ...
        relationships.children.forEach(function (child: any) {
            ...
            var rawBone = {
                ...
                indices: [],
                weights: [],
                transformLink: new FbxLoader.threeRef.Matrix4().fromArray(boneNode.TransformLink.a),
                // transform: new FbxLoader.threeRef.Matrix4().fromArray(boneNode.Transform.a),
    ...
    parseScene(deformers: any, geometryMap: any, materialMap: any) {
        sceneGraph = new FbxLoader.threeRef.Group();
        ...
        for (var nodeID in modelNodes) {
            ...
            if (! model) {
                switch (node.attrType) {
                    ...
                    case 'LimbNode':
                    case 'Root':
                        model = new FbxLoader.threeRef.Bone();
                        break;
                    case 'Null':
                    default:
                        model = new FbxLoader.threeRef.Group();
                        break;
                }
                model.name = (FbxLoader.threeRef.PropertyBinding as any).sanitizeNodeName(node.attrName);
                model.ID = id;
            }
            ...
            for (var ID in skeletons) {
                ...
                skeleton.rawBones.forEach(function (rawBone: any, i: number) {
                    if (rawBone.ID === parent.ID) {
                        var subBone = bone;
                        bone = new FbxLoader.threeRef.Bone();
                        bone.matrixWorld.copy(rawBone.transformLink);
                        // set name and id here - otherwise in cases where "subBone" is created it will not have a name / id
                        bone.name = (FbxLoader.threeRef.PropertyBinding as any).sanitizeNodeName(name);
                        bone.ID = id;
    ...
    createCamera(relationships: any) {
        ...
        if (cameraAttribute === undefined) {
            model = new FbxLoader.threeRef.Object3D();
        } else {
            var type = 0;
            ...
            switch (type) {
                case 0: // Perspective
                    model = new FbxLoader.threeRef.PerspectiveCamera(fov, aspect, nearClippingPlane, farClippingPlane);
                    if (focalLength !== null) model.setFocalLength(focalLength);
                    break;
                case 1: // Orthographic
                    model = new FbxLoader.threeRef.OrthographicCamera(- width / 2, width / 2, height / 2, - height / 2, nearClippingPlane, farClippingPlane);
                    break;
                default:
    ...
    createLight(relationships: any) {
        ...
        if (lightAttribute === undefined) {
            ...
            if (lightAttribute.Color !== undefined) {
                color = new FbxLoader.threeRef.Color().fromArray(lightAttribute.Color.value);
            }
            ...
            switch (type) {
                case 0: // Point
                    model = new FbxLoader.threeRef.PointLight(color, intensity, distance, decay);
                    break;
                case 1: // Directional
                    model = new FbxLoader.threeRef.DirectionalLight(color, intensity);
                    break;
                case 2: // Spot
                    var angle = Math.PI / 3;
                    if (lightAttribute.InnerAngle !== undefined) {
                        angle = FbxLoader.threeRef.Math.degToRad(lightAttribute.InnerAngle.value);
                    }
                    var penumbra = 0;
                    if (lightAttribute.OuterAngle !== undefined) {
                        ...
                        penumbra = FbxLoader.threeRef.Math.degToRad(lightAttribute.OuterAngle.value);
                        penumbra = Math.max(penumbra, 1);
                    }
                    model = new FbxLoader.threeRef.SpotLight(color, intensity, distance, angle, penumbra, decay);
                    break;
                default:
                    console.warn('THREE.FbxLoader: Unknown light type ' + lightAttribute.LightType.value + ', defaulting to a THREE.PointLight.');
                    model = new FbxLoader.threeRef.PointLight(color, intensity);
                    break;
            }
    ...
    createMesh(relationships: any, geometryMap: any, materialMap: any) {
        ...
        if (materials.length > 1) {
            ...
            material = new FbxLoader.threeRef.MeshPhongMaterial({color: 0xcccccc});
            materials.push(material);
        }
        if ('color' in geometry.attributes) {
            materials.forEach(function (material: any) {
                material.vertexColors = FbxLoader.threeRef.VertexColors;
            });
        }
        if (geometry.FBX_Deformer) {
            ...
            model = new FbxLoader.threeRef.SkinnedMesh(geometry, material);
            model.normalizeSkinWeights();
        } else {
            model = new FbxLoader.threeRef.Mesh(geometry, material);
        }
        return model;
    }
    createCurve(relationships: any, geometryMap: any) {
        ...
        var material = new FbxLoader.threeRef.LineBasicMaterial({color: 0x3300ff, linewidth: 1});
        return new FbxLoader.threeRef.Line(geometry, material);
    }
                ...
                if (child.relationship === 'LookAtProperty') {
                    ...
                    if ('Lcl_Translation' in lookAtTarget) {
                        ...
                        if (model.target !== undefined) {
                            ...
                            model.lookAt(new FbxLoader.threeRef.Vector3().fromArray(pos));
                        }
                    }
                }
            });
                ...
                if (geometryMap.has(parent.ID)) {
                    ...
                    geoRelationships.parents.forEach(function (geoConnParent: any) {
                        if (modelMap.has(geoConnParent.ID)) {
                            ...
                            model.bind(new FbxLoader.threeRef.Skeleton(skeleton.bones), bindMatrices[geoConnParent.ID]);
                        }
                    });
                }
            });
                ...
                if (BindPoseNode[nodeID].attrType === 'BindPose') {
                    ...
                    if (Array.isArray(poseNodes)) {
                        poseNodes.forEach(function (poseNode: any) {
                            bindMatrices[poseNode.Node] = new FbxLoader.threeRef.Matrix4().fromArray(poseNode.Matrix.a);
                        });
                    } else {
                        bindMatrices[poseNodes.Node] = new FbxLoader.threeRef.Matrix4().fromArray(poseNodes.Matrix.a);
                    }
                }
            }
        }
    ...
    createAmbientLight() {
        if ('GlobalSettings' in this.fbxTree && 'AmbientColor' in this.fbxTree.GlobalSettings) {
            ...
            if (r !== 0 || g !== 0 || b !== 0) {
                var color = new FbxLoader.threeRef.Color(r, g, b);
                sceneGraph.add(new FbxLoader.threeRef.AmbientLight(color, 1));
            }
        }
    }
    setupMorphMaterials() {
...
class GeometryParser {
    ...
    genGeometry(geoNode: any, skeleton: any, morphTarget: any, preTransform: any) {
        var geo = new FbxLoader.threeRef.BufferGeometry();
        if (geoNode.attrName) geo.name = geoNode.attrName;
        var geoInfo = this.parseGeoNode(geoNode, skeleton);
        var buffers = this.genBuffers(geoInfo);
        var positionAttribute = new FbxLoader.threeRef.Float32BufferAttribute(buffers.vertex, 3);
        preTransform.applyToBufferAttribute(positionAttribute);
        geo.addAttribute('position', positionAttribute);
        if (buffers.colors.length > 0) {
            geo.addAttribute('color', new FbxLoader.threeRef.Float32BufferAttribute(buffers.colors, 3));
        }
        if (skeleton) {
            geo.addAttribute('skinIndex', new FbxLoader.threeRef.Uint16BufferAttribute(buffers.weightsIndices, 4));
            geo.addAttribute('skinWeight', new FbxLoader.threeRef.Float32BufferAttribute(buffers.vertexWeights, 4));
            // used later to bind the skeleton to the model
            (geo as any).FBX_Deformer = skeleton;
        }
        if (buffers.normal.length > 0) {
            var normalAttribute = new FbxLoader.threeRef.Float32BufferAttribute(buffers.normal, 3);
            var normalMatrix = new FbxLoader.threeRef.Matrix3().getNormalMatrix(preTransform);
        ...
        buffers.uvs.forEach(function (_uvBuffer: any, i: number) {
            ...
            geo.addAttribute(name, new FbxLoader.threeRef.Float32BufferAttribute(buffers.uvs[i], 2));
        });
    ...
    genMorphGeometry(parentGeo: any, parentGeoNode: any, morphGeoNode: any, preTransform: any, name: string) {
        var morphGeo = new FbxLoader.threeRef.BufferGeometry();
        ...
        var positionAttribute = new FbxLoader.threeRef.Float32BufferAttribute(morphBuffers.vertex, 3);
    ...
    parseNurbsGeometry(geoNode: any) {
        ...
        if (isNaN(order)) {
            ...
            return new FbxLoader.threeRef.BufferGeometry();
        }
        var degree = order - 1;
        ...
        for (var i = 0, l = pointsValues.length; i < l; i += 4) {
            controlPoints.push(new FbxLoader.threeRef.Vector4().fromArray(pointsValues, i));
        }
        ...
        var geometry = new FbxLoader.threeRef.BufferGeometry();
        geometry.addAttribute('position', new FbxLoader.threeRef.BufferAttribute(positions, 3));
        return geometry;
    }
}
...
class AnimationParser {
                        ...
                        if (curveNode.curves.x !== undefined || curveNode.curves.y !== undefined || curveNode.curves.z !== undefined) {
                            if (layerCurveNodes[i] === undefined) {
                                ...
                                if (modelID !== undefined) {
                                    ...
                                    var node: any = {
                                        modelName: (FbxLoader.threeRef.PropertyBinding as any).sanitizeNodeName(rawModel.attrName),
                                        ID: rawModel.id,
                                    ...
                                    if (! node.transform) node.transform = new FbxLoader.threeRef.Matrix4();
    ...
    addClip(rawClip: any) {
        ...
        return new FbxLoader.threeRef.AnimationClip(rawClip.name, - 1, tracks);
    }
    generateTracks(rawTracks: any) {
        var tracks: any[] = [];
        var initialPosition = new FbxLoader.threeRef.Vector3();
        var initialRotation = new FbxLoader.threeRef.Quaternion();
        var initialScale = new FbxLoader.threeRef.Vector3();
        if (rawTracks.transform) rawTracks.transform.decompose(initialPosition, initialRotation, initialScale);
        var initialPositionArray = initialPosition.toArray();
        var initialRotationArray = new FbxLoader.threeRef.Euler().setFromQuaternion(initialRotation, rawTracks.eulerOrder).toArray();
    ...
    generateVectorTrack(modelName: string, curves: any, initialValue: any, type: string) {
        ...
        return new FbxLoader.threeRef.VectorKeyframeTrack(modelName + '.' + type, times, values);
    }
    generateRotationTrack(modelName: any, curves: any, initialValue: any, preRotation: any, postRotation: any, eulerOrder: any) {
        if (curves.x !== undefined) {
            ...
            curves.x.values = curves.x.values.map(FbxLoader.threeRef.Math.degToRad);
        }
        if (curves.y !== undefined) {
            this.interpolateRotations(curves.y);
            curves.y.values = curves.y.values.map(FbxLoader.threeRef.Math.degToRad);
        }
        if (curves.z !== undefined) {
            this.interpolateRotations(curves.z);
            curves.z.values = curves.z.values.map(FbxLoader.threeRef.Math.degToRad);
        }
        var times = this.getTimesForAllAxes(curves);
        var values = this.getKeyframeTrackValues(times, curves, initialValue);
        if (preRotation !== undefined) {
            preRotation = preRotation.map(FbxLoader.threeRef.Math.degToRad);
            preRotation.push(eulerOrder);
            preRotation = new FbxLoader.threeRef.Euler().fromArray(preRotation);
            preRotation = new FbxLoader.threeRef.Quaternion().setFromEuler(preRotation);
        }
        if (postRotation !== undefined) {
            postRotation = postRotation.map(FbxLoader.threeRef.Math.degToRad);
            postRotation.push(eulerOrder);
            postRotation = new FbxLoader.threeRef.Euler().fromArray(postRotation);
            postRotation = new FbxLoader.threeRef.Quaternion().setFromEuler(postRotation).inverse();
        }
        var quaternion = new FbxLoader.threeRef.Quaternion();
        var euler = new FbxLoader.threeRef.Euler();
        ...
        return new FbxLoader.threeRef.QuaternionKeyframeTrack(modelName + '.quaternion', times, quaternionValues);
    }
    generateMorphTrack(rawTracks: any) {
        ...
        return new FbxLoader.threeRef.NumberKeyframeTrack(rawTracks.modelName + '.morphTargetInfluences[' + morphNum + ']', curves.times, values);
    }
...
class BinaryReader {
    ...
    getString(size: number) {
        ...
        return FbxLoader.threeRef.LoaderUtils.decodeText(new Uint8Array(a));
    }
};
...
function generateTransform(transformData: any) {
    var tempEuler = new FbxLoader.threeRef.Euler();
    var tempVec = new FbxLoader.threeRef.Vector3();

    var lTranslationM = new FbxLoader.threeRef.Matrix4();
    var lPreRotationM = new FbxLoader.threeRef.Matrix4();
    var lRotationM = new FbxLoader.threeRef.Matrix4();
    var lPostRotationM = new FbxLoader.threeRef.Matrix4();
    var lScalingM = new FbxLoader.threeRef.Matrix4();
    var lScalingPivotM = new FbxLoader.threeRef.Matrix4();
    var lScalingOffsetM = new FbxLoader.threeRef.Matrix4();
    var lRotationOffsetM = new FbxLoader.threeRef.Matrix4();
    var lRotationPivotM = new FbxLoader.threeRef.Matrix4();
    var lParentGX = new FbxLoader.threeRef.Matrix4();
    var lGlobalT = new FbxLoader.threeRef.Matrix4();
    //var inheritType = (transformData.inheritType) ? transformData.inheritType : 0;
    if (transformData.translation) lTranslationM.setPosition(tempVec.fromArray(transformData.translation));
    if (transformData.preRotation) {
        var array = transformData.preRotation.map(FbxLoader.threeRef.Math.degToRad);
        array.push(transformData.eulerOrder);
        lPreRotationM.makeRotationFromEuler(tempEuler.fromArray(array));
    }
    if (transformData.rotation) {
        var array = transformData.rotation.map(FbxLoader.threeRef.Math.degToRad);
        array.push(transformData.eulerOrder);
        lRotationM.makeRotationFromEuler(tempEuler.fromArray(array));
    }
    if (transformData.postRotation) {
        var array = transformData.postRotation.map(FbxLoader.threeRef.Math.degToRad);
        array.push(transformData.eulerOrder);
    ...
    var lParentGRM = new FbxLoader.threeRef.Matrix4();
    ...
    */
    ...
    var lLocalTWithAllPivotAndOffsetInfo = new FbxLoader.threeRef.Matrix4().copyPosition(lTransform);
...
function convertArrayBufferToString(buffer: any, from?: number, to?: number) {
    ...
    return FbxLoader.threeRef.LoaderUtils.decodeText(new Uint8Array(buffer, from, to));
}
