...
import ThreeLoader from '../runtime_helper/interface/ThreeLoader';
import FbxLoader from '../runtime_helper/loaders/FbxLoader';
import TgaLoader from '../runtime_helper/loaders/TgaLoader';
...
export default class Three extends Importer {
   ...
   */
  public static threeRef: any = THREE;
  ...
  public getInitiator(name: string): (node: Node) => any {
    return (_node) => { return new Three.threeRef[name](); };
  }
  ...
  public hasInitiator(name: string): boolean {
    return Three.threeRef.hasOwnProperty(name);
  }
  ...
  public import(
    ...
    const root = new Three.threeRef.Group();
  ...
  public createRuntimeObject(node: Node, resources: any): any {
    ...
    if (node.meshRenderer && node.meshRenderer.mesh) {
      ...
      object = new Three.threeRef.Object3D();
    }
    ...
    if (!loader) {
      if (format === SupportedExportFormat.UNITY) {
        switch (type) {
          ...
          default: loader = new Three.threeRef.FileLoader() as ThreeLoader; break;
        }
      } else {
        loader = new Three.threeRef.FileLoader() as ThreeLoader;
      }
...
FbxLoader.threeRef = Three.threeRef;
TgaLoader.threeRef = Three.threeRef;
